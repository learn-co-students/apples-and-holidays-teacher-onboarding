...
def all_supplies_in_holidays(holiday_hash)
  ...
  holiday_hash.each do |season, season_hash|
    ...
    season_hash.each do |holiday, supplies|
      # holidays start out as symbols where the words are lowercase
      # and separated by underscores. this builds a list of uppercase strings
      holiday_word_list = holiday.to_s.split("_").map{|word| word.capitalize}
      puts "  #{holiday_word_list.join(" ")}: #{supplies.join(", ")}"
    end
  end
end
